...
class JwtTokenProviderTest {
  ...
  void issueAndVerifyAccessToken_Succeed_Test() {
    ...
    assertThat(now.minusMillis(parsed.issueAt().toEpochMilli()).toEpochMilli())
        .isLessThanOrEqualTo(skewSeconds * 1000);
    assertThat(expiry.minusMillis(parsed.expiredAt().toEpochMilli()).toEpochMilli())
        .isLessThanOrEqualTo(skewSeconds * 1000);
  }
  ...
  void expirationEqualsNow_AllowsParsing_CurrentImplementation() {
    ...
    String raw = provider.issueToken(claims).raw();
    assertThatThrownBy(
            () -> provider.verifyAndParseToken(raw, Date.from(now.plusSeconds(skewSeconds))))
        .isInstanceOf(TokenExpiredException.class);
  }
